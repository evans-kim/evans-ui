<template>
    <div class="relative">
        <ul ref="dropButton" style="min-height: 40px;" class="bg-white border border-main-200 flex flex-row flex-wrap h-full w-full select-none cursor-pointer">
            <li v-if="!hasValue"
            ...
            <li v-else v-for="item in getValue"
                ...
                class="text-sm shadow select-none hover:bg-gray-200 active:bg-gray-600 text-gray-800 border-gray-400 m-1 rounded ml-1 first:ml-2 border bg-gray-100 py-1 px-2">
                <slot name="result" :item="item">{{ getTag(item) }}</slot>
            </li>
        </ul>
        <div ref="dropMenu"
             :style="dropMenuStyle"
